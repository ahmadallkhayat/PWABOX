...
export function BookmarksSheet({ site, visible, currentUrl, onOpen, onRemove, onClose }: BookmarksSheetProps) {
  const { colors } = useTheme();
  const insets = useSafeAreaInsets();
  const onStartPage = currentUrl === site.url;

  return (
    <Sheet visible={visible} title={`${site.name} bookmarks`} onClose={onClose}>
      <FlatList
        data={site.bookmarks ?? []}
